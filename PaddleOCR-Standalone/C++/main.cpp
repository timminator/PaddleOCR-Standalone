...
void ocr(std::vector<cv::String> &cv_all_img_names) {
  ...
  for (int i = 0; i < cv_all_img_names.size(); ++i) {
    std::cout << "ppocr INFO: **********" << cv_all_img_names[i] << "**********" << std::endl;
    ...
    std::vector<std::vector<OCRPredictResult>> batch_results =
        ocr.ocr(img_batch, FLAGS_det, FLAGS_rec, FLAGS_cls);

    std::vector<OCRPredictResult> ocr_result;
    ocr_result = batch_results[0];
