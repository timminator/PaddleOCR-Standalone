...
void Utility::print_result(const std::vector<OCRPredictResult> &ocr_result) {
    if (ocr_result.empty()) {
        std::cout << "ppocr WARNING: No text found in image" << std::endl;
        return;
    }

    for (int i = 0; i < ocr_result.size(); ++i) {
        std::cout << "ppocr INFO: [[";
        for (size_t j = 0; j < ocr_result[i].box.size(); ++j) {
            std::cout << "[" << std::fixed << std::setprecision(1) 
                      << static_cast<float>(ocr_result[i].box[j][0]) << ", "
                      << static_cast<float>(ocr_result[i].box[j][1]) << "]";
            if (j != ocr_result[i].box.size() - 1) std::cout << ", ";
        }
        std::cout << "], ('" << ocr_result[i].text << "', " 
                  << std::fixed << std::setprecision(16) << ocr_result[i].score << ")]"
                  << std::endl;
    }
}
